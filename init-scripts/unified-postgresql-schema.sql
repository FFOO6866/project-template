...
-- =============================================================================
-- PRODUCT CLASSIFICATION TABLES (UNSPSC and ETIM)
-- =============================================================================
-- IMPORTANT: These tables contain REAL data from official sources only.
...
-- UNSPSC (United Nations Standard Products and Services Code) table
...
-- Indexes for UNSPSC codes
CREATE INDEX idx_unspsc_code ON unspsc_codes(code);
CREATE INDEX idx_unspsc_level ON unspsc_codes(level);
CREATE INDEX idx_unspsc_title ON unspsc_codes USING gin(to_tsvector('english', title));
CREATE INDEX idx_unspsc_segment ON unspsc_codes(segment) WHERE segment IS NOT NULL AND segment != '';
CREATE INDEX idx_unspsc_family ON unspsc_codes(family) WHERE family IS NOT NULL AND family != '';

-- Comment on UNSPSC table
COMMENT ON TABLE unspsc_codes IS 'UNSPSC classification codes - REAL data only (purchased from unspsc.org)';
COMMENT ON COLUMN unspsc_codes.code IS 'UNSPSC 8-digit code (e.g., 10101501 for Cattle)';
COMMENT ON COLUMN unspsc_codes.level IS '1=Segment, 2=Family, 3=Class, 4=Commodity';

-- ETIM (Electro-Technical Information Model) table
CREATE TABLE etim_classes (
    ...
    features JSONB, -- Technical features and characteristics
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (parent_class) REFERENCES etim_classes(class_code) ON DELETE SET NULL
);

-- Indexes for ETIM classes
CREATE INDEX idx_etim_class_code ON etim_classes(class_code);
CREATE INDEX idx_etim_version ON etim_classes(version);
CREATE INDEX idx_etim_parent ON etim_classes(parent_class);
CREATE INDEX idx_etim_description_en ON etim_classes USING gin(to_tsvector('english', description_en));
CREATE INDEX idx_etim_description_de ON etim_classes USING gin(to_tsvector('german', description_de)) WHERE description_de IS NOT NULL;
CREATE INDEX idx_etim_features ON etim_classes USING gin(features) WHERE features IS NOT NULL;

-- Comment on ETIM table
COMMENT ON TABLE etim_classes IS 'ETIM classification classes - REAL data only (requires ETIM membership)';
COMMENT ON COLUMN etim_classes.class_code IS 'ETIM class code (e.g., EC000001)';
COMMENT ON COLUMN etim_classes.features IS 'JSONB of technical features and characteristics';

-- Product classifications linking table
CREATE TABLE product_classifications (
    ...
    classification_method VARCHAR(50), -- 'manual', 'ai', 'rules', 'import'
    classified_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    classified_by VARCHAR(100), -- User ID or system component
    notes TEXT,
...
-- Indexes for product classifications
CREATE INDEX idx_product_classifications_product ON product_classifications(product_id);
CREATE INDEX idx_product_classifications_unspsc ON product_classifications(unspsc_code);
CREATE INDEX idx_product_classifications_etim ON product_classifications(etim_class);
CREATE INDEX idx_product_classifications_method ON product_classifications(classification_method);
CREATE INDEX idx_product_classifications_confidence ON product_classifications(confidence);

-- Comment on product classifications
COMMENT ON TABLE product_classifications IS 'Links products to UNSPSC and ETIM classifications with confidence scores';
COMMENT ON COLUMN product_classifications.confidence IS 'Classification confidence score (0.0-1.0)';
COMMENT ON COLUMN product_classifications.classification_method IS 'How classification was determined: manual, ai, rules, import';

-- Create triggers for updated_at columns
CREATE TRIGGER update_unspsc_codes_updated_at BEFORE UPDATE ON unspsc_codes FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_etim_classes_updated_at BEFORE UPDATE ON etim_classes FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- =============================================================================
-- AI RECOMMENDATION ENGINE TABLES
-- =============================================================================

-- Category keyword mappings for recommendation engine
...
CREATE INDEX idx_category_keyword ON category_keyword_mappings(keyword);
CREATE INDEX idx_category_category ON category_keyword_mappings(category);

COMMENT ON TABLE category_keyword_mappings IS 'Keyword-to-category mappings for AI recommendation engine';
COMMENT ON COLUMN category_keyword_mappings.priority IS 'Higher priority keywords are weighted more in matching (1=default)';

-- Task keyword mappings for recommendation engine
...
CREATE INDEX idx_task_keyword ON task_keyword_mappings(keyword);
CREATE INDEX idx_task_task_id ON task_keyword_mappings(task_id);

COMMENT ON TABLE task_keyword_mappings IS 'Keyword-to-task mappings for knowledge graph task extraction';
COMMENT ON COLUMN task_keyword_mappings.priority IS 'Higher priority keywords are weighted more in matching (1=default)';
